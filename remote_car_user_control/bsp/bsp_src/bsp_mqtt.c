...
{
    /* Always use zero qos for fast speed and false retain */
    int res = esp_mqtt_client_publish(esp_mqtt_client_handle, topic, data, data_len, BSP_MQTT_QOS, BSP_MQTT_RETAIN);

    if(res >= 0)
    {
        ESP_LOGI(TAG, "publish is successful code: %d", res);
    }
    else if(res == -1)
    {
        ESP_LOGE(TAG, "subscribe is failure: -1");
